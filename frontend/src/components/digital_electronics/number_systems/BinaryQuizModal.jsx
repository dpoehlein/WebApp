...
const BinaryQuizModal = ({ isOpen, onClose }) => {
    ...
    const generateBinaryToDecimal = () => {
        const dec = Math.floor(Math.random() * (255 - 10 + 1)) + 10; // 10 to 255
    ...
    const handleStartQuiz = () => {
        const generated = [
            ...Array(3).fill(null).map(() => generateBinaryToDecimal()),
            ...Array(3).fill(null).map(() => generateDecimalToBinary()),
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
            <div className="bg-white rounded-xl shadow-lg p-6 max-w-3xl w-full overflow-y-auto max-h-[90vh]">
                            ...
                            <div key={i}>
                                ...
                                {submitted && (
                                    <p className={`mt-1 text-sm ${answers[i]?.trim() === q.correctAnswer ? "text-green-600" : "text-red-600"}`}>
                                        {answers[i]?.trim() === q.correctAnswer ? "✅ Correct" : `❌ Correct Answer: ${q.correctAnswer}`}
                                    </p>
                                )}
                            </div>
                        ))}
                ...
                <div className="mt-6 flex justify-end">
                    <button onClick={onClose} className="text-gray-600 hover:text-black font-semibold">Close</button>
                </div>
            </div>
        </div>
    );
